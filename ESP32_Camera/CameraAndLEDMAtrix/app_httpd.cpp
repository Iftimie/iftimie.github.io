...
static bool is_valid_seq(const char* s) {
  if (!s) return false;

  bool has_end = false;
  for (const char* p = s; *p; p++) {
    char c = *p;
    if (c == 'F' || c == 'B' || c == 'L' || c == 'R') continue;
    if (c >= '0' && c <= '9') continue;
    if (c == ' ' || c == ',' || c == '\t' || c == '\r' || c == '\n') continue;
    if (c == ';') { has_end = true; continue; }
    return false; // reject anything else
  }
  return has_end; // require ';' so ATtiny knows the batch ends
}
...
static esp_err_t move_handler(httpd_req_t *req) {
  ...
  // read entire query string: "seq=F50L10;"
  ...
  char seq[256]; // max accepted length
  if (httpd_query_key_value(buf, "seq", seq, sizeof(seq)) != ESP_OK) {
    free(buf);
  ...
  if (!is_valid_seq(seq)) {
    httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Invalid seq");
    return ESP_FAIL;
  }

  log_e("Move sequence: %s", seq);
  // Send to ATtiny over UART
  Serial2.print(seq);
